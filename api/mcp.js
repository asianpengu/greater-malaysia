...
const TOOLS = [
  ...
  { name: "get_public_holidays", description: "Malaysian public holidays for a year, plus the next upcoming one. Pass state for the national + state union. Source-cited.", inputSchema: { type: "object", properties: { year: { type: "string", description: "Four-digit year. Default 2026." }, state: { type: "string", description: "Optional state / federal territory code: jhr, kdh, ktn, mlk, nsn, phg, prk, pls, png, sbh, swk, sgr, trg, kul, lbn, pjy." } }, additionalProperties: false } },
...
const ROUTES = {
  ...
  get_public_holidays: (a) => `/api/holidays?year=${encodeURIComponent(a.year || "2026")}${a.state ? `&state=${encodeURIComponent(a.state)}` : ""}`,
